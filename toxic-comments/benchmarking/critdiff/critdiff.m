function [cd,r] = cd(s,labels,alpha,clique)
%
...
for i=1:ceil(k/2)
   ...
   h = text(1.2, 101 - 5*(n+1)- 10*i + 2, num2str(r(i)), 'FontSize', 10, 'HorizontalAlignment', 'right');

   text(1.25, 100 - 5*(n+1) - 10*i, labels{idx(i)}, 'FontSize', 12, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left');
...
for i=ceil(k/2)+1:k
   ...
   text(-0.2, 101 - 5*(n+1) -10*(k-i+1)+2, num2str(r(i)), 'FontSize', 10, 'HorizontalAlignment', 'left');

   text(-0.25, 100 - 5*(n+1) -10*(k-i+1), labels{idx(i)}, 'FontSize', 12, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'right');
