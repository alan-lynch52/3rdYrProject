...
def main():
    ...
    #split train into train and target
    y = train.iloc[:,2:]
    ...
    #split test into feature set and test ids
    ...
    d['Binary'] = bin_train
    benchmark("TF-IDF-Base",model, d['TF-IDF-Base'], y)
    benchmark("Count-Best",model, d['Count-Best'], y)
    benchmark("Binary",model, d['Binary'], y)
    get_balanced_accuracy_fe(d,y)
    get_auroc_fe(d,y)
    plot_cm_fe(d,y)

    #old parameter tuning
    ...
    ##tfidf_preds = get_probability(train_tfidf, y, test_tfidf, test_ids)
    ##count_preds = get_probability(count_train, y, count_test, test_ids)
    ##bin_preds = get_probability(bin_train, y, bin_test, test_ids)
